...
class Request():
	def __init__(self, _json, latency) -> None:
		...
		if 'postData' in _json:
			...
			params = post_data['params'] if 'application/json' not in self.content_type else None
	...
	def format_request(self):
		...
		self.payload = self._zip_dict_arr(self.payload) if self.payload and type(self.payload) != dict else self.payload
		self.query_dict = self._zip_dict_arr(self.query_dict) if len(self.query_dict) > 0 else {}

		if 'cookie' in self.headers:
			self.headers.pop('cookie')

class Config():
	def __init__(self, har_file, url_dict={}, headers_dict={}, param_dict={}, payload_dict={}, hook_class=None, allow_redirects=True, _async_file_reqs=False, fingerprint_threshold=None):
...
class SATFramework():
	...
	def _get_stats(self, request_index):
		...
		progress = f'{request_index}/{len(self.reference_data)}'

		latency = self._wrap_color(latency, '\033[0;32m')
		progress = self._wrap_color(progress, '\033[1;36m')

		if self.request.is_auxillary:
			return progress

		return f'{(f"{latency} ") if self.request.latency > 0 else str()}{progress}'
	...
	def get_response_size(self, response):
		...
		response_size = response_content_size + self._get_dict_size(self.request.headers)
	...
	def get_next_request(self, index):
		...
		next_req = self.reference_data[index + 1] if index != (len(self.reference_data) - 1) else _json

		latency = (datetime.strptime(next_req['startedDateTime'][:-6], '%Y-%m-%dT%H:%M:%S.%f') - datetime.strptime(_json['startedDateTime'][:-6], '%Y-%m-%dT%H:%M:%S.%f')).microseconds / 100000
	...
	def hook_request(self, request: Request):
		...
		request.headers = url_tools.modify_dict(request.headers, self.headers_dict, hook_class=self.hook_class)
		...
		if request.payload:
			request.payload = url_tools.modify_dict(request.payload, self.payload_dict, hook_class=self.hook_class)
	...
	def hook_response(self, response: requests.Response):
		if self.hook_class:
			...
			functions = [getattr(self.hook_class, attr) for attr in dir(self.hook_class) if not(attr.startswith('__')) and callable(getattr(self.hook_class, attr))]
	...
	def make_request(self):
		...
		request_func_dict = {
			...
			'OPTIONS': lambda: self.session.options(url, allow_redirects=self.allow_redirects),
			'POST': lambda: self.session.post(url, json=payload, allow_redirects=self.allow_redirects) if payload and 'application/json' in self.request.content_type else (self.session.post(url, allow_redirects=self.allow_redirects) if not payload else self.session.post(url, data=self.request.payload, allow_redirects=self.allow_redirects))
		}
